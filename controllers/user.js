...
module.exports = function(formidable, passport, validation, email, User) {
	return {
		...
		router.get('/nlp', this.nlp);
		router.post('/nlp/fetch', this.nlpApi);
	...
	nlpApi: function(req, res) {
		const textApi = new Aylien({
			application_id: "68837b99",
			application_key: "0c18469543844e03b477be1a884e87bc"
		});
		
		textApi.sentiment({ url: req.body.url }, (error, result, remaining) => {
			res.send(result);
		});
	},
	...
	photosView: function(req, res) {
		...
		//let photos = ['https://www.fillmurray.com/640/360', 'https://dummyimage.com/640x360/fff/aaa'];
		photos = photos.map((photo, index) => ({src: photo, index: index}) );
